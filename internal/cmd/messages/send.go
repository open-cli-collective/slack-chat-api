...
func runSend(channel, text string, opts *sendOptions, c *client.Client) error {
	...
	// Handle file uploads
	if hasFiles {
		return uploadFiles(c, channel, text, opts)
	}
...
func uploadFiles(c *client.Client, channel, text string, opts *sendOptions) error {
	...
	err := c.CompleteUploadExternal(uploadedFiles, channel, opts.threadTS, text)
	if err != nil {
